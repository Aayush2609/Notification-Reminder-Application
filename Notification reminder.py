...
t = Tk()
t.title('Notification Reminder')
t.geometry("500x300")
img = Image.open("notification1.jpg")
img = img.resize((500,65), Image.ANTIALIAS)
tkimage = ImageTk.PhotoImage(img)
t.iconphoto(False, tkimage)

#get details
def get_details():
    print(f"The title of the notification is:{title.get()}")
    print(f"The message to be notified is:{msg.get()}")
    get_title = title.get()
    get_msg = msg.get()
    get_time = time1.get()

    if get_title == "" or get_msg == "" or get_time == "":
        messagebox.showerror("Alert", "Fill the required fields!")
    else:
